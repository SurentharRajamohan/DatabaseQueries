...
CREATE TABLE RESEARCH_ISSUE(
   ...
   Notes VARCHAR(50),
   PRIMARY KEY (CaseID, LawJournalID, IssueID),
...
SELECT * FROM CLIENTS WHERE ClientID = 7001;
SELECT * FROM CASES WHERE CaseDescription = 'Land Registration';
SELECT * FROM LAWYERS WHERE LawyerID = 8001;
SELECT * FROM ACCOUNTANTS WHERE AccountantID = 'A001';
SELECT * FROM APPOINTMENT WHERE CompanyName = 'ANW';
SELECT * FROM LAW_JOURNAL WHERE TypeOfJournal = 'Malayan Law Journal';
SELECT * FROM ISSUES WHERE IssueDescription = 'Murder';
SELECT * FROM EVIDENCE WHERE EvidenceID = 5001;
SELECT * FROM VERDICT WHERE Judgement = 'Guilty';
SELECT * FROM RESEARCH_ISSUE WHERE Notes = 'Insurance overdue';
SELECT * FROM CLIENT_ACCOUNTS WHERE Status = 'Unpaid';
SELECT * FROM OFFICE_ACCOUNTS WHERE TagID = 1001;
