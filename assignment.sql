...
(TagID, AccountDescription, Amount)
VALUES 
(1001, 'Legal Fee Income', 500000.00);
INSERT INTO OFFICE_ACCOUNTS 
(TagID, AccountDescription, Amount)
VALUES 
(1002, 'Website', 800.00);
...
(TagID, AccountDescription, Amount)
VALUES 
(1003, 'Internet Bill', 400.00);
INSERT INTO OFFICE_ACCOUNTS 
(TagID, AccountDescription, Amount)
VALUES 
(1004, 'Salaries', 250000.00);
INSERT INTO OFFICE_ACCOUNTS 
(TagID, AccountDescription, Amount)
VALUES 
(1005, 'Rent Bill', 6000.00);
INSERT INTO OFFICE_ACCOUNTS 
(TagID, AccountDescription, Amount)
VALUES 
(1006, 'Utilities', 2500.00);
INSERT INTO OFFICE_ACCOUNTS 
(TagID, AccountDescription, Amount)
VALUES 
(1007, 'Email marketing', 800.00);
INSERT INTO OFFICE_ACCOUNTS 
(TagID, AccountDescription, Amount)
VALUES 
(1008, 'Travel Expense', 2000.00);
...
(
   SELECT ClientID FROM Client_Accounts WHERE Status = 'Unpaid'
);
...
--Integrity checking
    ...
    SELECT COUNT(*) FROM ACCOUNTANTS GROUP BY AccountantID HAVING COUNT(*) > 1;

    --This query will return the number of duplicate rows in the table based on the primary key column. 
    --If the query returns no rows, then the primary key constraint is being upheld.
    SELECT AccountantID, COUNT(*) FROM Accountants GROUP BY AccountantID HAVING COUNT(*) > 1;
    ...
    SELECT ClientID FROM Client_Accounts WHERE ClientID NOT IN (SELECT ClientID FROM Clients);
...
--MAX
SELECT CLIENTS.FirstName, CLIENTS.LastName, CLIENT_ACCOUNTS.AMOUNT
FROM CLIENTS
JOIN CLIENT_ACCOUNTS
ON CLIENTS.ClientID = CLIENT_ACCOUNTS.CLIENTID
WHERE CLIENTS.ClientID = (
    SELECT CLIENTID FROM CLIENT_ACCOUNTS
    WHERE AMOUNT = (SELECT MAX(AMOUNT) FROM CLIENT_ACCOUNTS)
);
...
WHERE CLIENTS.ClientID = (
    SELECT CLIENTID FROM CLIENT_ACCOUNTS
    WHERE AMOUNT = (SELECT MIN(AMOUNT) FROM CLIENT_ACCOUNTS)
);
...
/*
Correlated subqueries: A correlated subquery is a subquery that references a column from the outer query. 
The subquery is executed once for each row of the outer query and its results are used to filter the rows of the outer query.
